...
.. tip::
   ...
   *UpperCamelCase* (beginning with a capital letter, then
   ...
   overview of the naming conventions in appendix A, *Coding
   Guidelines*.
...
TYPO3 distribution and lie in the folder :file:`typo3/sysext/`.
...
the entire process of listing creation later on. The folder :file:`Domain` again contains the two folders
:file:`Model` and
:file:`Repository`. Resulting from all this, the folder structure within the extension folder
:file:`store_inventory` should look like in image 4-1.
...
After the basic structure has been constructed, the extension can already be shown in the extension manager and
